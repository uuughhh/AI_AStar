...
class Map_Obj():
    ...
    def isWalkable (self,pos:list[int,int]) -> bool:
        """Check if a position is walkable"""
        if (self.get_cell_value(pos) == -1):
            return False
        else:
             return True

    def get_neighbors(self,pos: list[int, int]) -> list:
        """
        Getter for the neighbors of the current position
    ...
    def set_walked (self,pos:list[int,int]):
        """
        setter function to mark a position as walked on the map.
